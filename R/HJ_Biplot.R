...
{
  hjb = list(loadings = NULL, coord_ind = NULL, coord_var = NULL,
  ...
  colnames(hjb$eigenvalues) = PCs
  hjb$explvar = t(as.matrix(round(d^2/sum(d^2), digits = 4) *
                              100))
  colnames(hjb$explvar) = PCs
  ...
  plot(hjb$coord_ind, col = "blue", xlab = eje1, ylab = eje2,
       pch = 19, xlim = c(xmin, xmax), ylim = c(ymin, ymax))
  arrows(0, 0, hjb$coord_var[, 1], hjb$coord_var[, 2], col = "green4",
         length = 0.1, angle = 20, lwd = 2)
  abline(h = 0, v = 0, col = "gray30", lwd = 1, lty = 2)
  if (ind_name == TRUE) {
    i = 1
    for (tag in ind_tag) {
      text(hjb$coord_ind[i, 1] + 0.2, hjb$coord_ind[i,
                                                    2] + 0.2, labels = ind_tag[i], col = "blue",
           cex = 0.65, font = 2)
      i = i + 1
    }
  }
