...
#'
#' @export
LASSO_HJBiplot = function (X, lambda, transform_data = "scale", operator = "Hard-Thresholding",
                           ind_name = FALSE, vec_name = TRUE)
{
  hj_lasso = list(loadings = NULL, n_ceros = NULL, coord_ind = NULL,
                  coord_var = NULL, eigenvalues = NULL, explvar = NULL)
  ind_tag = rownames(X)
  vec_tag = colnames(X)
  ...
  xmin = min(hj_lasso$coord_ind[, 1], hj_lasso$coord_var[,
                                                         1]) - 0.5
  xmax = max(hj_lasso$coord_ind[, 1], hj_lasso$coord_var[,
                                                         1]) + 0.5
  ymin = min(hj_lasso$coord_ind[, 2], hj_lasso$coord_var[,
                                                         2]) - 0.5
  ymax = max(hj_lasso$coord_ind[, 2], hj_lasso$coord_var[,
                                                         2]) + 0.5
  ...
  plot(hj_lasso$coord_ind, col = "navy", xlab = axis1, ylab = axis2,
       pch = 20, xlim = c(xmin, xmax), ylim = c(ymin, ymax))
  arrows(0, 0, hj_lasso$coord_var[, 1], hj_lasso$coord_var[,
                                                           2], length = 0.1, angle = 20, col = "red")
  abline(h = 0, v = 0, col = "gray30", lwd = 1, lty = 2)
  if (ind_name == TRUE) {
    i = 1
    for (tag in ind_tag) {
      text(hj_lasso$coord_ind[i, 1] + 0.2, hj_lasso$coord_ind[i,
                                                              2] + 0.2, labels = ind_tag[i], col = "navy",
           cex = 0.75, font = 2, offset = 0.01)
      i = i + 1
    }
  }
  if (vec_name == TRUE) {
